...
link_weather = "https://api.openweathermap.org/data/2.5/weather?lat=-23.467788&lon=-47.439033&appid=4f2dae55f9cbc3025870e1e389f249e2"
...
email.send_keys("guilhermewaizbart@gmail.com")
sleep(1)
password = d.find_element_by_xpath('//*[@id="pass"]')
password.send_keys("gui4107314")
sleep(1)
...
while True:
    try:
        try:
            #weather
            req = requests.get(link_weather).json()
            ...
            write.writerow([time, float(uberx_price.text.replace("R$", "").replace(",", ".")), float(comfort_price.text.replace("R$", "").replace(",", ".")), weather])
            file.close() 
        except:
            file.close()
